...
angular.module('Notifications', [])
	.factory('notificationFactory', ['$timeout', function ($timeout) {
		...
		var settings = {
			...
			warning:  { duration: 10000, enabled: true },
...
//          console.log('notifications are not supported');
			...
			info: function (id, title, content, userData) {
				return this.awesomeNotify(id, 'info', 'info', title, content, userData);
			},

			error: function (id, title, content, userData) {
				return this.awesomeNotify(id, 'error', 'remove', title, content, userData);
			},

			success: function (id, title, content, userData) {
				return this.awesomeNotify(id, 'success', 'ok', title, content, userData);
			},

			warning: function (id, title, content, userData) {
				return this.awesomeNotify(id, 'warning', 'exclamation', title, content, userData);
			},

			awesomeNotify: function (id, type, icon, title, content, userData) {
				/**
				...
				return this.makeNotification(id, type, false, icon, title, content, userData);
			},

			notify: function (id, image, title, content, userData) {
				// Wraps the makeNotification method for displaying notifications with images
				// rather than icons
				return this.makeNotification(id, 'custom', image, true, title, content, userData);
			},

			makeNotification: function (id, type, image, icon, title, content, userData) {
				var notification = {
					'type': type,
					'image': image,
					'icon': icon,
...
//    console.log('this is a new directive');
		var html =
			'<div class="alert" ng-repeat="noti in queue" ng-animate="\'show-alert\'">' +
